...
type
  ...
  private
    ...
    FTime: Integer;                     //Время смены базового цвета
  public
    procedure Init;
    procedure DeInit;
...
begin
  //Настройки графики
  Graphic.Capabilities[gcColorBlend] := True;
  ...
  //Настройка системы
  TickDelay := 500;
  FTime := Round((1000 / TickDelay) * 300);
  TickEnable := True;
end;
...
var
  ...
  a: Single;
begin
  //Изменить базовый цвет
  if TickCount mod FTime = 0 then
    FColor := sgeGraphicColor_GetColor(Random, Random, Random, 1);
