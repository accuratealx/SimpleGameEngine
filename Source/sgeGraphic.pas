...
type
  ...
  TsgeGraphicCapabilities = (gcPointSmooth, gcLineSmooth, gcPolygonSmooth, gcColorBlend, gcLineStipple, gcTexture, gcScissor);
  ...
  private
    ...
    procedure SetCapabilities(ACapabilities: TsgeGraphicCapabilities; AEnable: Boolean);
    function  GetCapabilities(ACapabilities: TsgeGraphicCapabilities): Boolean;
  ...
  public
    ...
    procedure DrawSpritePart(X, Y, W, H: Single; Xs, Ys, Ws, Hs: Word; Sprite: TsgeGraphicSprite; Mode: TsgeGraphicDrawMode = gdmNormal);
    procedure DrawSpritePart(X, Y, W, H: Single; Xs, Ys, Ws, Hs: Word; Sprite: TsgeGraphicSprite; Angle, Scale: Single; Mode: TsgeGraphicDrawMode = gdmNormal);
    procedure DrawSpritePart(X, Y: Single; Xs, Ys, Ws, Hs: Word; Sprite: TsgeGraphicSprite; Mode: TsgeGraphicDrawMode = gdmNormal);
    procedure DrawSpritePart(X, Y: Single; Xs, Ys, Ws, Hs: Word; Sprite: TsgeGraphicSprite; Angle, Scale: Single; Mode: TsgeGraphicDrawMode = gdmNormal);
    ...
    property Capabilities[Index: TsgeGraphicCapabilities]: Boolean read GetCapabilities write SetCapabilities;
...
procedure TsgeGraphic.SetCapabilities(ACapabilities: TsgeGraphicCapabilities; AEnable: Boolean);
begin
  case ACapabilities of
    gcPointSmooth  : if AEnable then glEnable(GL_POINT_SMOOTH) else glDisable(GL_POINT_SMOOTH);
    gcLineSmooth   : if AEnable then glEnable(GL_LINE_SMOOTH) else glDisable(GL_LINE_SMOOTH);
    gcPolygonSmooth: if AEnable then glEnable(GL_POLYGON_SMOOTH) else glDisable(GL_POLYGON_SMOOTH);
    gcColorBlend   : if AEnable then glEnable(GL_BLEND) else glDisable(GL_BLEND);
    gcLineStipple  : if AEnable then glEnable(GL_LINE_STIPPLE) else glDisable(GL_LINE_STIPPLE);
    gcTexture      : if AEnable then glEnable(GL_TEXTURE_2D) else glDisable(GL_TEXTURE_2D);
    gcScissor      : if AEnable then glEnable(GL_SCISSOR_TEST) else glDisable(GL_SCISSOR_TEST);
  end;
end;


function TsgeGraphic.GetCapabilities(ACapabilities: TsgeGraphicCapabilities): Boolean;
begin
  case ACapabilities of
    gcPointSmooth  : Result := glIsEnabled(GL_POINT_SMOOTH);
    gcLineSmooth   : Result := glIsEnabled(GL_LINE_SMOOTH);
    gcPolygonSmooth: Result := glIsEnabled(GL_POLYGON_SMOOTH);
    gcColorBlend   : Result := glIsEnabled(GL_BLEND);
    gcLineStipple  : Result := glIsEnabled(GL_LINE_STIPPLE);
    gcTexture      : Result := glIsEnabled(GL_TEXTURE_2D);
    gcScissor      : Result := glIsEnabled(GL_SCISSOR_TEST);
  end;
end;
...
begin
  ...
  if wglSwapIntervalEXT <> nil then wglSwapIntervalEXT(0);    //Попробывать выключить вертикальную синхронизацию
...
procedure TsgeGraphic.DrawSpritePart(X, Y, W, H: Single; Xs, Ys, Ws, Hs: Word; Sprite: TsgeGraphicSprite; Mode: TsgeGraphicDrawMode);
var
  Rect, sRect: TsgeGraphicRect;
begin
  Rect := GetNormalRect(X, Y, W, H, Mode);
...
procedure TsgeGraphic.DrawSpritePart(X, Y, W, H: Single; Xs, Ys, Ws, Hs: Word; Sprite: TsgeGraphicSprite; Angle, Scale: Single; Mode: TsgeGraphicDrawMode);
var
  Rect, sRect: TsgeGraphicRect;
begin
  Rect := GetTransformedRect(X, Y, W, H, Mode);
...
procedure TsgeGraphic.DrawSpritePart(X, Y: Single; Xs, Ys, Ws, Hs: Word; Sprite: TsgeGraphicSprite; Mode: TsgeGraphicDrawMode);
begin
  if Mode = gdmClassic then Mode := gdmNormal;
...
procedure TsgeGraphic.DrawSpritePart(X, Y: Single; Xs, Ys, Ws, Hs: Word; Sprite: TsgeGraphicSprite; Angle, Scale: Single; Mode: TsgeGraphicDrawMode);
begin
  if Mode = gdmClassic then Mode := gdmNormal;
...
var
  ...
  F: TFileStream;
  Width, Height, BytesPerLine, Trash, Size: Integer;
...
begin
  ...
  BFH.bfReserved1 := 0;
  BFH.bfReserved2 := 0;
  BFH.bfOffBits := SizeOf(TBitmapFileHeader) + SizeOf(TBITMAPINFOHEADER); //Смещение от начала файла до самих данных
  BFH.bfSize := BFH.bfOffBits + Size;                 //Размер файла целиком со структурами и мусором

  //Описатель BMP
  BIH.biSize := SizeOf(TBITMAPINFOHEADER);            //Размер этой структуры. Интересно зачем
