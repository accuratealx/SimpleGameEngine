...
uses
  sgeTypes, sgeMemoryStream, sgeFile, sgePackFile, sgePackFileReader, sgeStringList,
  LazUTF8, LCLIntf;


const
  FormTitle = 'SGEPACK [v 0.4]';
  _UNITNAME = 'MainUnit';
...
begin
  if FModified then
    ...
    try
      ...
      try
        F := TsgeFile.Create(TempFn, fmWrite);
        F.Size := 0;
    ...
    if Cancel then
      ...
      if FileExists(Fn) then DeleteFile(Fn);  //Удалить старый файл проекта
      RenameFile(TempFn, Fn);                 //Заменить файл проекта новым
