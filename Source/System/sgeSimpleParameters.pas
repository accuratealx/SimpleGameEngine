...
begin
  ...
  c := GetCount - 1;
  for i := 0 to c do
    if UTF8LowerCase(FParamList[i].Name) = Name then
      begin
      Result := i;
      Break;
      end;
...
begin
  ...
  Idx := IndexOf(Name);
  if Idx <> -1 then
    if TryStrToInt(FParamList[Idx].Value, Val) then Result := Val;
end;
...
begin
  ...
  Idx := IndexOf(Name);
  if Idx <> -1 then
    if TryStrToFloat(FParamList[Idx].Value, Val) then Result := Val;
end;
...
begin
  ...
  if Idx <> -1 then
    ...
    TrueStr := UTF8LowerCase(TrueStr);
    FalseStr := UTF8LowerCase(FalseStr);
    Val := UTF8LowerCase(FParamList[Idx].Value);
...
begin
  ...
  if not TryStrToInt(FParamList[Idx].Value, i) then
...
begin
  ...
  if not TryStrToFloat(FParamList[Idx].Value, R) then
    raise EsgeException.Create(_UNITNAME, Err_UnableToDetermineValue, FParamList[Idx].Value);
...
begin
  ...
  cName := UTF8LowerCase(FParamList[Idx].Value);

  //True
  if cName = UTF8LowerCase(TrueStr) then
    begin
    Result := True;
    Exit;
    end;
  ...
  //Flase
  if cName = UTF8LowerCase(FalseStr) then
    begin
    Result := False;
    Exit;
    end;
...
var
  ...
  Found: Boolean;
begin
  //Создать список и зугрузить из строки
  ...
  for i := 0 to c do
    ...
    Param1 := UTF8LowerCase(FParamList[i].Name);
    ...
    for j := 0 to k do
      ...
      //Сравнить параметры
      if Param1 = UTF8LowerCase(Trim(Param2)) then
        begin
        Found := True;
        Break;
        end;
...
begin
  ...
  for i := 0 to c do
    begin
    S := Trim(List.Part[i]);                //Отрезать мусор
    ...
    //Обработать параметр
    sParam := UTF8Trim(sParam);
    if sParam = '' then Continue;
    sValue := UTF8Trim(sValue);
