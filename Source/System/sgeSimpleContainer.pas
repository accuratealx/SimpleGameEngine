...
uses
  sgeConst, sgeTypes, sgeStringList, sgeSimpleParameters, sgeMemoryStream;
...
begin
  ...
  if not (soCaseSensivity in FSearchOptions) then Name := UTF8LowerCase(Name);
  ...
  for i := 0 to c do
    begin
    //Проверить модификатор поиска
    if not (soCaseSensivity in FSearchOptions) then s := UTF8LowerCase(FSectionList[i].Name) else s := FSectionList[i].Name;
...
begin
  ...
  List.SearchOptions := [];
  List.FromString(UTF8Trim(FSectionList[Idx].Value));
...
begin
  ...
  ParamName := UTF8LowerCase(Parameter);
  Found := False;
  c := List.Count - 1;
  for i := 0 to c do
    begin
    ...
    if ParamName = UTF8LowerCase(UTF8Trim(pName)) then
      begin
...
begin
  SetSectionParameter(Name, Parameter, IntToStr(Value));
end;


procedure TsgeSimpleContainer.SetSectionParameter(Name: String; Parameter: String; Value: Real);
begin
  SetSectionParameter(Name, Parameter, FloatToStr(Value));
end;
...
begin
  ...
  for i := 0 to c do
    ...
    cValue := '';
    IsSet := False;
    SectionName := UTF8LowerCase(FSectionList[i].Name);
      ...
      case State of
        ...
        sValue:
          ...
          if S = StapleClose then
            ...
            if UTF8LowerCase(UTF8Trim(cName)) = SectionName then
              begin
  ...
  for i := 1 to Length(Str) do
    ...
    case State of
      ...
      sValue:
        ...
        if S = StapleClose then
          begin
          Add(UTF8Trim(cName), UTF8Trim(cValue));
          cName := '';
          cValue := '';
          State := sNormal;
          Continue;
