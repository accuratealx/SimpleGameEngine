...
function sgeGraphicFont_AttribToString(Attrib: TsgeGraphicFontAttrib): String;
function sgeGraphicFont_StringToAttrib(AttribStr: String): TsgeGraphicFontAttrib;
...
begin
  ...
  if Pos('b', AttribStr) > 0 then Include(Result, gfaBold);
  if Pos('i', AttribStr) > 0 then Include(Result, gfaItalic);
  if Pos('u', AttribStr) > 0 then Include(Result, gfaUnderline);
  if Pos('s', AttribStr) > 0 then Include(Result, gfaStrikeOut);

  //Особый случай
  if Pos('[]', AttribStr) > 0 then Result := [];
end;
...
begin
  //Параметры шрифта
  ErrStr := FName + ' ' + IntToStr(FHeight) + ' ' + sgeGraphicFont_AttribToString(FAttrib);
