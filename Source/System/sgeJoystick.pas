...
begin
  ...
  if joyGetPos(ID, @J) <> JOYERR_NOERROR then
    raise EsgeException.Create(_UNITNAME, Err_DeviceNotAttach, IntToStr(ID));

  //Запросить параметры
  if joyGetDevCaps(ID, @FCaps, SizeOf(TJOYCAPS)) <> JOYERR_NOERROR then
    raise EsgeException.Create(_UNITNAME, Err_CantGetDeviceInfo, IntToStr(ID));
...
begin
  ...
  if joyGetPosEx(FDriverID, @FPosEx) <> JOYERR_NOERROR then
    raise EsgeException.Create(_UNITNAME, Err_CantReadData, IntToStr(FDriverID));
