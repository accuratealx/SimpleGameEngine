...
type
  ...
  //Время события
  TsgeFadePassedTime = (fptBegin, fptMiddle, fptEnd);

  //Функция обработчик прошедшего времени
  TsgeFadeProc = procedure(Time: TsgeFadePassedTime) of Object;
  ...
  private
    ...
    FCallBackProc: TsgeFadeProc;  //Обработчик прошедшего времени
    ...
    procedure ValuesPrepare(Mode: TsgeFadeMode; Color: TsgeGraphicColor);

    procedure DoCallBack(Time: TsgeFadePassedTime);
  public
    destructor Destroy; override;

    procedure Start(Mode: TsgeFadeMode; Color: TsgeGraphicColor; Time: Cardinal; Proc: TsgeFadeProc = nil);
    procedure Stop;
...
procedure TsgeFade.ValuesPrepare(Mode: TsgeFadeMode; Color: TsgeGraphicColor);
begin
...
procedure TsgeFade.DoCallBack(Time: TsgeFadePassedTime);
begin
...
procedure TsgeFade.Start(Mode: TsgeFadeMode; Color: TsgeGraphicColor; Time: Cardinal; Proc: TsgeFadeProc);
begin
  //Восстановить флажки
  FTimeBegin := False;
  FTimeMiddle := False;
...
begin
  ...
  if cTime <= FTime then
    ...
    if not FTimeMiddle and (Pos >= 0.5) then
      begin
      FTimeMiddle := True;
      DoCallBack(fptMiddle);
      end;

    Result := sgeGraphicColor_ChangeAlpha(FColor, ValuesGradient(cTime / FTime))
    end
    else begin
    DoCallBack(fptEnd);
    FEnable := False;
