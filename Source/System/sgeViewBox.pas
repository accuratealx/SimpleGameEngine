...
type
  ...
  private
    ...
    FMapSize: TsgeGraphicPoint;                             //Размеры карты

    FScreenOffset: TsgeGraphicPoint;                        //Смещение левого-верхнего угла экрана относительно левого-вехнего угла карты
    FScreenCenter: TsgeGraphicPoint;                        //Координаты центра экрана относительно карты

    FScreenSize: TsgeGraphicPoint;                          //Размеры экрана
  ...
  public
    ...
    property MapSize: TsgeGraphicPoint read FMapSize write SetMapSize;
    ...
    property ScreenSize: TsgeGraphicPoint read FScreenSize write SetScreenSize;
...
procedure TsgeViewBox.SetMapSize(ASize: TsgeGraphicPoint);
begin
  SetMapWidth(ASize.X);
...
begin
  ...
  if W > FScreenSize.X then
    ...
    if FScreenOffset.X + FScreenSize.X >= W then FScreenOffset.X := W - FScreenSize.X;
    end else FScreenOffset.X := -((FScreenSize.X / 2) - (W / 2));
...
begin
  ...
  if H > FScreenSize.Y then
    ...
    if FScreenOffset.Y + FScreenSize.Y >= H then FScreenOffset.Y := H - FScreenSize.Y;
    end else FScreenOffset.Y := -((FScreenSize.Y / 2 - H / 2));
...
procedure TsgeViewBox.SetScreenOffset(AOffset: TsgeGraphicPoint);
begin
...
procedure TsgeViewBox.SetScreenCenter(ACenter: TsgeGraphicPoint);
begin
...
procedure TsgeViewBox.SetScreenSize(ASize: TsgeGraphicPoint);
begin
...
begin
  ...
  if FScaleType = vbstDependent then
    ...
    if rScreen > rMap then scl := FScreenSize.X / FMapSize.X
      else scl := FScreenSize.Y / FMapSize.Y;
  ...
  if W <= FScreenSize.X then
    ...
    case Safe of
      True:
        ...
        if Result >= FMapSize.X then Result := FMapSize.X;
        end;

      False:
        if (AX >= 0) and (AX <= W) then Result := AX / FScale else Result := -1;
    end;

    end else Result := (FScreenOffset.X + AX) / FScale;
end;
  ...
  if H <= FScreenSize.Y then
    ...
    case Safe of
      True:
        ...
        Result := AY / FScale;
        if Result <= 0 then Result := 0;
        if Result >= FMapSize.Y then Result := FMapSize.Y;
        end;

      False:
        if (AY >= 0) and (AY <= H) then Result := AY / FScale else Result := -1;
    end;

    end else Result := (FScreenOffset.Y + AY) / FScale;
end;
...
function TsgeViewBox.GetMapBounds: TsgeGraphicRect;
var
  Pt: TsgeGraphicPoint;
begin
