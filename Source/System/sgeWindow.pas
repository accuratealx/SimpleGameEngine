...
begin
  ...
  case APos of
    wcpScreen:
      begin
      W := Windows.GetSystemMetrics(SM_CXSCREEN);         //Размер первого экрана
      H := Windows.GetSystemMetrics(SM_CYSCREEN);
      end;
    wcpClientArea:
      begin
      W := Windows.GetSystemMetrics(SM_CXFULLSCREEN);     //Клиенстская область рабочего стола
      H := Windows.GetSystemMetrics(SM_CYFULLSCREEN);
      end;
    wcpVirtualScreen:
      begin
      W := Windows.GetSystemMetrics(SM_CXVIRTUALSCREEN);  //Размер всех мониторов
