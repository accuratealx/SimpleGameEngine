...
begin
  ...
  //Проверить загрузку
  if Bmp = 0 then
    raise EsgeException.Create(_UNITNAME, Err_CantLoadBitmap, BmpName);

  //Узнать размеры
  if GetObject(Bmp, SizeOf(TBITMAP), @b) = 0 then
    raise EsgeException.Create(_UNITNAME, Err_CantGetBitmapData, BmpName);
  ...
  try
    ...
    on E: EsgeException do
      raise EsgeException.Create(_UNITNAME, Err_CantCreateWindow, E.Message);
  end;
  ...
  FWindow.WindowWidth := b.bmWidth;
  FWindow.WindowHeight := b.bmHeight;
  FWindow.Center;
end;
