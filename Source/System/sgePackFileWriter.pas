...
uses
  sgeTypes, sgeConst, sgeMemoryStream, sgeFile, sgePackFile, sgePackFileReader,
  SysUtils, LazUTF8;


const
  _UNITNAME = 'sgePackFileWriter';
...
begin
  Result := -1;
  c := GetCount - 1;
  s := UTF8LowerCase(PackName);

  for i := 0 to c do
    if UTF8LowerCase(FFileList[i].PackName) = s then
      begin
      Result := i;
      Break;
      end;
    ...
    try
      ...
      for i := 0 to c do
        ...
        on E: EsgeException do
          raise EsgeException.Create(_UNITNAME, Err_FileWriteError, FileName, E.Message);
      end;
