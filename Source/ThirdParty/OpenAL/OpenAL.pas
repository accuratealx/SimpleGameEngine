(* Творческий человек 17.08.2018 г.  (accuratealx@gmail.com)

  На Lazarus 1.8.4 и FPC 3.0.4 не работает из коробки.
  Или переводили давно, или не хватило знаний, хотя в
  коде комментарии от некоего Benjamin Rosseaux (BeRo),
  говорят о том, что всё было адаптировано под FPC.
  Но, зачем было подключать 'C.dll' вместо стандарных
  функций GetProcAdress, я так и не понял. В итоге
  компилятор страшно ругался, что не находит c.dll.
...
begin
  Result := nil;
  if Addr(alGetProcAddress) <> nil then Result := alGetProcAddress(ProcName);
...
begin
  ...
  Pointer(alGetProcAddress) := GetProcAddress(LibHandle, 'alGetProcAddress');

  if (LibHandle <> 0) then
  begin

    Pointer(alEnable) := alProcedure('alEnable');
    Pointer(alDisable) := alProcedure('alDisable');
    Pointer(alIsEnabled) := alProcedure('alIsEnabled');
    Pointer(alHint) := alProcedure('alHint');
    Pointer(alGetBooleanv) := alProcedure('alGetBooleanv');
    Pointer(alGetIntegerv) := alProcedure('alGetIntegerv');
    Pointer(alGetFloatv) := alProcedure('alGetFloatv');
    Pointer(alGetDoublev) := alProcedure('alGetDoublev');
    Pointer(alGetString) := alProcedure('alGetString');
    Pointer(alGetBoolean) := alProcedure('alGetBoolean');
    Pointer(alGetInteger) := alProcedure('alGetInteger');
    Pointer(alGetFloat) := alProcedure('alGetFloat');
    Pointer(alGetDouble) := alProcedure('alGetDouble');
    Pointer(alGetError) := alProcedure('alGetError');
    Pointer(alIsExtensionPresent) := alProcedure('alIsExtensionPresent');
    Pointer(alGetEnumValue) := alProcedure('alGetEnumValue');

    Pointer(alListenerf) := alProcedure('alListenerf');
    Pointer(alListener3f) := alProcedure('alListener3f');
    Pointer(alListenerfv) := alProcedure('alListenerfv');

    Pointer(alListeneri) := alProcedure('alListeneri');
    Pointer(alListener3i) := alProcedure('alListener3i');
    Pointer(alListeneriv) := alProcedure('alListeneriv');

    Pointer(alGetListeneriv) := alProcedure('alGetListeneriv');
    Pointer(alGetListenerfv) := alProcedure('alGetListenerfv');

    Pointer(alGenSources) := alProcedure('alGenSources');
    Pointer(alDeleteSources) := alProcedure('alDeleteSources');
    Pointer(alIsSource) := alProcedure('alIsSource');

    Pointer(alSourcei) := alProcedure('alSourcei');
    Pointer(alSource3i) := alProcedure('alSource3i');
    Pointer(alSourceiv) := alProcedure('alSourceiv');

    Pointer(alSourcef) := alProcedure('alSourcef');
    Pointer(alSource3f) := alProcedure('alSource3f');
    Pointer(alSourcefv) := alProcedure('alSourcefv');

    Pointer(alGetSourcei) := alProcedure('alGetSourcei');
    Pointer(alGetSource3i) := alProcedure('alGetSource3i');
    Pointer(alGetSourceiv) := alProcedure('alGetSourceiv');

    Pointer(alGetSourcef) := alProcedure('alGetSourcef');
    Pointer(alGetSource3f) := alProcedure('alGetSource3f');
    Pointer(alGetSourcefv) := alProcedure('alGetSourcefv');

    Pointer(alSourcePlay) := alProcedure('alSourcePlay');
    Pointer(alSourcePause) :=alProcedure('alSourcePause');
    Pointer(alSourceStop) := alProcedure('alSourceStop');
    Pointer(alSourceRewind) := alProcedure('alSourceRewind');
    Pointer(alSourcePlayv) := alProcedure('alSourcePlayv');
    Pointer(alSourceStopv) := alProcedure('alSourceStopv');
    Pointer(alSourceRewindv) := alProcedure('alSourceRewindv');
    Pointer(alSourcePausev) := alProcedure('alSourcePausev');
    Pointer(alGenBuffers) := alProcedure('alGenBuffers');
    Pointer(alDeleteBuffers) := alProcedure('alDeleteBuffers');
    Pointer(alIsBuffer) := alProcedure('alIsBuffer');
    Pointer(alBufferData) := alProcedure('alBufferData');

    Pointer(alBufferi) := alProcedure('alBufferi');
    Pointer(alBuffer3i) := alProcedure('alBuffer3i');
    Pointer(alBufferiv) := alProcedure('alBufferiv');

    Pointer(alBufferf) := alProcedure('alBufferf');
    Pointer(alBuffer3f) := alProcedure('alBuffer3f');
    Pointer(alBufferfv) := alProcedure('alBufferfv');

    Pointer(alGetBufferi) := alProcedure('alGetBufferi');
    Pointer(alGetBuffer3i) := alProcedure('alGetBuffer3i');
    Pointer(alGetBufferiv) := alProcedure('alGetBufferiv');

    Pointer(alGetBufferf) := alProcedure('alGetBufferf');
    Pointer(alGetBuffer3f) := alProcedure('alGetBuffer3f');
    Pointer(alGetBufferfv) := alProcedure('alGetBufferfv');

    Pointer(alSourceQueueBuffers) := alProcedure('alSourceQueueBuffers');
    Pointer(alSourceUnqueueBuffers) := alProcedure('alSourceUnqueueBuffers');
    Pointer(alDistanceModel) := alProcedure('alDistanceModel');
    Pointer(alDopplerFactor) := alProcedure('alDopplerFactor');
    Pointer(alDopplerVelocity) := alProcedure('alDopplerVelocity');
    Pointer(alSpeedOfSound) := alProcedure('alSpeedOfSound');

    Pointer(alcCreateContext) := alProcedure('alcCreateContext');
    Pointer(alcMakeContextCurrent) := alProcedure('alcMakeContextCurrent');
    Pointer(alcProcessContext) := alProcedure('alcProcessContext');
    Pointer(alcSuspendContext) := alProcedure('alcSuspendContext');
    Pointer(alcDestroyContext) := alProcedure('alcDestroyContext');
    Pointer(alcGetError) := alProcedure('alcGetError');
    Pointer(alcGetCurrentContext) := alProcedure('alcGetCurrentContext');
    Pointer(alcOpenDevice) := alProcedure('alcOpenDevice');
    Pointer(alcCloseDevice) := alProcedure('alcCloseDevice');
    Pointer(alcIsExtensionPresent) := alProcedure('alcIsExtensionPresent');
    Pointer(alcGetProcAddress) := alProcedure('alcGetProcAddress');
    Pointer(alcGetEnumValue) := alProcedure('alcGetEnumValue');
    Pointer(alcGetContextsDevice) := alProcedure('alcGetContextsDevice');
    Pointer(alcGetString) := alProcedure('alcGetString');
    Pointer(alcGetIntegerv) := alProcedure('alcGetIntegerv');

    Pointer(alcCaptureOpenDevice) := alProcedure('alcCaptureOpenDevice');
    Pointer(alcCaptureCloseDevice) := alProcedure('alcCaptureCloseDevice');
    Pointer(alcCaptureStart) := alProcedure('alcCaptureStart');
    Pointer(alcCaptureStop) := alProcedure('alcCaptureStop');
    Pointer(alcCaptureSamples) := alProcedure('alcCaptureSamples');
...
begin
  if (LibHandle <> 0) then
    begin
      ...
      begin
        Pointer(EAXSet) := alProcedure('EAXSet');
        Pointer(EAXGet) := alProcedure('EAXGet');
      end;
